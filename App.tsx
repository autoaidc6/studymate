...
import { KeyStage, StudyMaterial } from './types';
...
const App: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    if (!selectedKeyStage || !topic.trim()) {
      setError("Please select a Key Stage and enter a topic.");
      return;
    }
    ...
    try {
      const result = await generateStudyGuide(selectedKeyStage, topic);
  ...
  }, [selectedKeyStage, topic]);

  const canGenerate = selectedKeyStage !== null && topic.trim().length > 0;
      ...
      <main className="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="max-w-3xl mx-auto bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-2xl shadow-lg">
          <div className="space-y-6">
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 mt-2">
                {KEY_STAGES.map((ks) => (
                  <button
                    key={ks.id}
                    onClick={() => setSelectedKeyStage(ks)}
            ...
            <div>
              <label htmlFor="topic" className="text-lg font-medium text-slate-700 dark:text-slate-200">2. Enter a Topic</label>
              <input
                id="topic"
                type="text"
                value={topic}
