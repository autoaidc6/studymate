...
interface HeaderProps {
  ...
  activeModule: 'GUIDE' | 'LEARN';
  onModuleChange: (module: 'GUIDE' | 'LEARN') => void;
}

const Header: React.FC<HeaderProps> = ({ onShowSavedGuides, activeModule, onModuleChange }) => {
  return (
    <header className="py-6 px-4 sm:px-6 lg:px-8 no-print border-b border-slate-200 dark:border-slate-800">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <div className="flex items-center justify-center space-x-8">
          ...
          <nav className="hidden md:flex items-center space-x-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
            <button
              onClick={() => onModuleChange('GUIDE')}
